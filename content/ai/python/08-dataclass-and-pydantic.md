...
关键价值：**校验失败的报错是结构化、带字段路径的**，写重试逻辑（让模型照着报错修正 JSON）时直接把错误信息传给模型即可。
...
两者可共存：核心领域模型用 dataclass，边界处用 pydantic 校验后再转 dataclass；小项目全程 pydantic 也没问题。pydantic v2 用 Rust 重写了校验核心，性能不再是顾虑。
