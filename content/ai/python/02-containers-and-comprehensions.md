...
date: 2026-09-05
tags: [Python]
summary: list / tuple / dict / set 的选择逻辑、切片、推导式，以及浅拷贝这个所有新手都要踩一次的坑。
---

Python 日常代码 80% 的数据操作发生在四种内置容器上。这篇讲清楚"什么场景用哪个"，以及推导式这个 Python 的标志性语法。
...
决策口诀：**要键值用 dict，要唯一性用 set，结构固定用 tuple，其余用 list**。`in` 判断在 list 上是线性扫描，在 set/dict 上是常数时间——数据量大时换容器比换算法更有效：
...
推导式不是炫技，是把"过滤 + 变换"表达成一个短句。超过两层嵌套或需要 `else` 分支嵌套时，就退回普通 for 循环——可读性优先。

生成器推导式 `(x for x in xs)` 不立即物化，适合大数据流（与生成器篇章呼应）。
...
规则：`=` 是别名，`[:]` / `list()` / `dict()` 是浅拷贝，嵌套结构要 `copy.deepcopy`。写 RAG 代码里 chunk 列表、检索结果传递时，这个坑会以"改了 A 结果 B 也变了"的形式出现。
