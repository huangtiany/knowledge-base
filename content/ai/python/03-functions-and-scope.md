...
函数是 Python 组织代码的最小单元。这篇覆盖参数系统、lambda、作用域规则，最后收在两个经典坑上。
...
`*` 之后的参数**强制关键字传参**——LLM SDK 的函数几乎都这么设计（参数太多，位置传参会错位）。反过来 `*args, **kwargs` 收集任意参数：
...
需要多行、需要文档字符串的，就写 `def`——lambda 的存在意义就是"这里要一个函数，但不值得命名"。
...
闭包是装饰器（见[装饰器、with 与生成器](09-decorators-context-generators.md)）的实现基础。要在内层**重新绑定**外层变量，必须声明 `nonlocal`（改外层函数的变量）或 `global`（改模块变量）——不声明的话赋值动作会创建一个新的局部变量，这是最常见的静默 bug。

## 坑一：可变默认参数

```python title="mutable-default.py"
def add_msg(msg, bag=[]):       # ❌ 默认值只创建一次！
    bag.append(msg)
    return bag
...
闭包捕获的是变量本身，不是当时的值。循环里造闭包，用默认参数把值"冻"进去。
