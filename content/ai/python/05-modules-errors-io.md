...
基础段收尾篇：把代码组织成模块、优雅地处理出错、格式化输出、读写文件。这四件事撑起之后所有工程代码的骨架。
...
`__name__ == "__main__"` 让一个文件既能当模块被导入、又能当脚本直接跑——几乎所有工程代码的入口都长这样。

## 异常：EAFP 风格

Python 的品味是 **EAFP**（Easier to Ask Forgiveness than Permission）：先做，出错了再接。对比 LBYL（Look Before You Leap，先判断再做）：
...
print(f"{n:,}")                                 # 1,024 千分位
print(f"{score=}")                              # score=0.8734 —— 调试神器
print(f"{n:>6}")                                # 右对齐补空格
```

Python 3.12 前 f-string 里不能嵌套同样的引号；3.12 起随意。老代码里的 `%` 和 `.format()` 能读懂即可，新代码一律 f-string。
