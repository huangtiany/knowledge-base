---
title: asyncio：并发调用 LLM 的刚需
date: 2026-09-05
tags: [Python]
summary: 事件循环、协程、gather 与信号量限流——把"串行调 100 次 embedding"变成"十几秒全部返回"的那把钥匙。
---

LLM 应用是典型的 **IO 密集**：一次 embedding 调用大部分时间在等网络回包，CPU 在干等。asyncio 用单线程事件循环把"等待"重叠起来，100 个请求的吞吐能差出一个数量级。事件循环模型可以直接对照 JS 的 async/await——概念同构，语法几乎一样，区别在 Python 显式区分协程函数与普通函数。

## 协程三件套：async def / await / asyncio.run
...
Python 的特别之处：**忘了 await 不会报错，只会拿到一个"什么都没发生"的协程对象**——看到 `<coroutine object ...> never awaited` 警告就是这回事。JS 里 `await` 可选导致的问题一样存在，只是报错形态不同。
...
`gather` 传一组协程，全部完成后按**原顺序**返回结果列表。3.11+ 更推荐 `asyncio.TaskGroup`（一个失败取消全部，异常处理更干净）；批量场景 gather 简单够用。注意别用 `time.sleep()`——它会阻塞整个事件循环，协程世界里一切等待都要 await 版本。

## 信号量限流：并发数不是越多越好

API 有速率限制，无脑 1000 并发只会收获 429：
...
async def embed_all(texts: list[str], concurrency: int = 10):
    ...
    async def worker(text: str) -> list[float]:
        async with sem:                            # 拿到"名额"才执行
            return await embed_one(text)

    return await asyncio.gather(*(worker(t) for t in texts))
```

`async with sem` 保证任意时刻只有 N 个协程在等响应，其余排队。LLM 应用并发抓取、批量 embedding、跑评测集，这个模式直接抄。

## 什么时候不用 asyncio

- CPU 密集（本地推理、大矩阵运算）：事件循环帮不上忙，那是 multiprocessing/向量化的事
- 脚本就发一两个请求：直接用同步 SDK，别为一把小刀引入整套异步
- 用到异步库就得全链路异步：`async def` 里调同步阻塞函数（如 `time.sleep`、同步 HTTP 客户端）会卡死循环——SDK 选型时确认有 async 版本（openai.AsyncOpenAI、httpx.AsyncClient）
