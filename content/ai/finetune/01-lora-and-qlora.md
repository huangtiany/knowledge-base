...
date: 2026-09-05
tags: [微调]
summary: 全参微调 7B 要 A100，LoRA 只要 24G 消费卡：冻结原权重，只训低秩增量。开源模型的微调生态主要建立在这个方法上。
---

[训练侧全景](../llm-basics/05-pretraining-to-rlhf.md)说过：微调改行为不改知识。微调在工程上的主流形态是 LoRA/QLoRA，它们把微调的门槛从"数据中心"降到了"一张消费级显卡"。
...
1. **问题能用 prompt 解决吗**：few-shot + 结构化输出（见[结构化输出](../llm-basics/04-structured-output-internals.md)）能解决的不微调。微调是重投入，且一旦实施就直接改变模型行为
2. **要改的是知识还是行为**：新事实/新文档 → RAG（见[RAG 章](../rag/01-chunking-strategies.md)）；固定格式、风格、领域话术、稳定流程 → 微调
3. **有几百条以上高质量样本吗**：数据少于百条，收益大概率抵不过过拟合风险（数据准备见[数据集准备](03-dataset-preparation.md)）
4. **评测基线建好了吗**：微调前后跑同一套评测（见[评测先行](../evaluation/01-evaluation-first.md)），否则"感觉变好"无从验证
