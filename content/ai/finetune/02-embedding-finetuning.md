...
date: 2026-09-05
tags: [微调]
summary: 当通用 embedding 模型在你的领域分不清相似文档时，用检索对比学习微调它：数据可由 LLM 合成、训练成本低、能直接提升 RAG 检索命中率。
---
...
- hit@k 评测里，**该命中的 chunk 排名总在 5~20 名**（相关但压不过相似干扰项），典型的领域语义区分度不足信号
...
```text
正例来源：把领域文档交给 LLM，"为这段文档生成 3 个用户会问的问题"
...
要点：**批次内其他样本自动充当负例**（in-batch negatives，所以 batch_size 不宜太小）；lr 用模型推荐的小学习率；训练后**必须跑同一评测集对比**，旧模型 92% → 新模型 96% 才算有效，同时抽查通用查询是否有退化（灾难性遗忘）。
