...
date: 2026-09-05
tags: [LLM基础]
summary: 窗口的容量按 token 计算，token 既费钱又占空间；算准 token 用量、合理安排截断，是 RAG 和 Agent 控制成本与质量的前提。
---

"128K 上下文"听上去能装下一本书，实际使用中先会遇到三个问题：**算不准 token**、**中间内容被忽略**、**费用失控**。下文逐一说明成因与对策。
...
不同模型词表不同，跨模型对比时用对应 tokenizer。在线速查可用 OpenAI/HF 的 tokenizer playground。粗估换算：**中文 1 字 ≈ 1 token，英文 1 词 ≈ 1.3 token，JSON 再上浮 30%**。

## 窗口的真实结构：prompt 与输出共享预算

上下文窗口（如 128K）是 prompt + 输出**共享**的预算，而且各段的处理方式不同：

- **system prompt**：每请求都发、每请求都计费，常驻
- **对话历史**：逐轮累积，是长对话费用失控的主因
- **检索内容（RAG）**：每请求注入的大块文本
- **输出**：预留出 max_tokens 的空间

关键事实（"lost in the middle"现象，2023 年起多个研究复现）：模型对**开头和结尾**的上下文利用率最高，**中段**内容容易被忽略。所以 RAG 组装 prompt 时：指令放最前，**最重要的检索结果放最后（紧贴生成位置）**，而不是按相关性分数简单排序。
...
2. **选内容**（RAG 场景）：top_k 少放几条、每条 chunk 截短；或先 rerank 再取前几条（见[检索与重排](../rag/04-retrieval-and-rerank.md)）。窗口预算优先分配给最相关的内容
3. **硬截断**：最后手段。截在句子或段落边界，不要截在词中间；截完在结尾标一句"（前文已截断）"让模型知情
...
计费按输入 + 输出 token 分开算，输入通常是输出的几倍到几十倍（RAG 里每请求都重发检索内容）。三个降本手段按性价比排：**缓存**（同样的 system/前缀，各家用 KV cache 折扣或显式 prompt cache）、**少塞**（rerank 后少放 chunk）、**换模型**（简单任务路由到小模型）。
