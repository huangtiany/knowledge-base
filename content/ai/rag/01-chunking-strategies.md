...
date: 2026-09-05
tags: [RAG]
summary: RAG 效果的天花板在切块就定了：切坏了，后面 embedding、检索、生成全在给一个错误输入擦屁股。
---

RAG 管线的第一步是把文档切成 chunk（块）。一句话定位它的分量：**embedding 模型是给 chunk 做表示的，chunk 边界错了，语义就碎了，后面全链路在错误的输入上优化**。这也是"RAG 效果不好先查切块"的原因。

## 核心矛盾：一个 chunk 要同时服务两个主人
...
**4. 语义切分**：按句子算 embedding，相邻句相似度骤降处断开——"语义边界"就是话题转换处。效果好但要多跑一遍 embedding，成本高；当默认档调不动再上。
...
- **只按 chunk_size 数数**：把"3.2.1 节标题"和它的正文切成两块，标题块的向量毫无意义 → 结构感知或加 overlap
- **表格被切散**：表格切碎后行列关系全丢 → 表格整块保留，或转成描述文本
- **元数据丢失**：切块后不记录来源文档/章节/页码 → 生成时的引用溯源（见[生成与引用](05-generation-and-citations.md)）没有数据可用

## 调优方法：先建评测集

切块参数（大小/overlap/策略）没有万能默认值，正确姿势是固定一小批"问题→标准出处"评测对，换参数跑命中率——这就是[评测先行](../evaluation/01-evaluation-first.md)在 RAG 的第一个应用。
