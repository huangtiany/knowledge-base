...
date: 2026-09-05
tags: [RAG]
summary: 纯向量检索输在"精确词匹配"，hybrid 检索补上，rerank 负责最后的精排——这套"粗召回 + 精排序"的两段式是现代 RAG 的标准架构。
---

前两篇解决了"向量从哪来、存到哪"，这篇解决"怎么查得准"。现代 RAG 检索的标准架构是两段式：**召回（recall）求全，重排（rerank）求准**。
...
根因：向量检索擅长"意思相近"，不擅长"字面精确"。补法是把字面匹配请回来。
...
经验比例：**向量 + BM25 常态起手**；代码/型号/术语密集的语料，BM25 权重该更高。
...
为什么 cross-encoder 更准：双塔（bi-encoder）把 query 和 doc **各自独立**编码成向量，交互信息有限；cross-encoder 让两者在模型内部逐层交互，能捕捉细粒度的匹配关系。代价是每对都要过一次模型，算不动全库——所以只能当"精排"，不能当"召回"。

**双塔召回 + 交叉重排**的本质：把"快而糙"和"慢而准"各用在刀刃上。这一架构从搜索引擎时代就是标准答案，RAG 只是重新发明了它。
