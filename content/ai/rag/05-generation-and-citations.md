...
date: 2026-09-05
tags: [RAG]
summary: 检索只是一半，生成侧决定用户看到什么：上下文怎么摆、引用怎么标、"没找到就说没找到"怎么写进 prompt。
---

检索到了好的 chunk，生成侧做错三件事照样失败：上下文摆放失当、没有引用机制、prompt 不允许模型说"不知道"。
...
两级引用架构的取舍：**段落级引用**（标 chunk 编号）简单可靠，是默认；**句子级引用**（quote 逐字对应原文）可信度更高但要后验校验 + 重试，成本明显上升。落地顺序：先段落级，用户反馈"不敢信"再升级。

## 幻觉缓解：三道防线

幻觉 = 模型输出了资料里没有/与资料矛盾的内容。防线按层：

1. **prompt 层**：显式授权"可以说不知道"，不给出路模型就自己编。上面的模板规则 1 是必写项
2. **生成参数层**：抽取/引用场景 `temperature` 压到 0~0.3（见[采样参数](../llm-basics/01-sampling-parameters.md)）
3. **校验层**：生成后机器校验，覆盖引用编号存在性、quote 对原文的包含性，必要时让另一个 LLM 当裁判比对答案与资料（faithfulness 评测，见 [RAGAS](../evaluation/02-ragas.md)）

**根因排查优先于防线堆叠**：答案错时先问"是不是检索没找对"（hit 不中）还是"检索对了但生成歪了"（生成层幻觉），两者的处理方式完全不同。这正是评测要把 retrieval 和 generation 分开测的原因。
