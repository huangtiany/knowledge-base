...
**经验锚点**：百万条 1024 维向量，内存占用约 4~6 GB（fp32），HNSW 检索毫秒级——多数个人/团队项目的语料量级根本到不了"库的性能成为瓶颈"的程度。选型错误更常发生在**过滤能力和数据形态**上，而不是 QPS 上。

## 三个候选的取舍

**Chroma**——嵌入式优先。`pip install chromadb` 后直接在进程内用，数据落本地目录。原型期最佳体验；但分布式、高级过滤、权限都弱，"项目成功后再迁移"是默认剧本。

**Qdrant**——为生产而生。payload 过滤（结构化元数据 + 向量混合查询）设计得最顺，Rust 实现，内存/磁盘两种模式，有云服务也能 docker 自托管。RAG 服务化的稳妥默认项：
...
- 元数据要**入库时就作为 payload 存好**（来源、日期、语言、章节）——切块篇说的元数据丢失，到了这里没救
