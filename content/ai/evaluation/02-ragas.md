...
date: 2026-09-05
tags: [评测与可观测]
summary: faithfulness 检查答案是否忠于检索内容，answer relevancy 检查是否答到问题，context precision/recall 检查检索是否给对了材料。四个指标把 RAG 的失败定位到具体环节。
---

[评测先行](01-evaluation-first.md)说了要分层测，RAGAS 是把"分层"工程化的标准工具：用 LLM-as-judge 把 RAG 的失败定位到检索或生成的具体环节。下文说明四个核心指标的语义、计算方式与噪声控制。

## 四个指标的分工

RAGAS 把一次 RAG 回答拆成「检索 → 生成」两段分别检查：

| 指标 | 问的问题 | 用到的数据 | 失败定位 |
|---|---|---|---|
| **Context Precision** | 检索到的 chunk 里，相关的排前面了吗 | query + 检索结果 | 检索/排序 |
| **Context Recall** | 该有的信息都检索到了吗 | query + 检索结果 + 标准答案 | 检索/切块/embedding |
| **Faithfulness** | 答案是否忠于检索内容（不编造） | 检索结果 + 答案 | 生成侧 |
| **Answer Relevancy** | 答案答到问题要点了吗 | query + 答案 | 生成/prompt |

典型读法（这是 RAGAS 最大的价值）：

- faithfulness 低、context recall 高 → **检索没问题，生成在编造**：压 temperature、加"不知道就明说"、上引用校验（见[生成与引用](../rag/05-generation-and-citations.md)）
- context recall 低 → **问题出在入库或检索**：查 chunking、换 embedding、补 hybrid（检索层对策，见[检索与重排](../rag/04-retrieval-and-rerank.md)）
- answer relevancy 低但 faithfulness 高 → 答案忠于材料但没答到要点，改 prompt/改检索 query，不是幻觉问题
...
- **同一 judge 配置跑全程**：一次优化周期内 judge 配置冻结，否则分数不可比
- **绝对值不做硬性解读，趋势才可靠**：faithfulness 从 0.72 → 0.85 是真改进；"0.85 高不高"没有标准答案
