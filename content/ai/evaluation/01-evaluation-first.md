...
date: 2026-09-05
tags: [评测与可观测]
summary: 换 embedding 后指标是升是降？新 prompt 是否更好？没有一组固定的评测对，这些问题只能靠感觉。评测先行的最小实践：50 个用例、三个指标、一次基线。
---

这是横切章的第一篇。核心原则：**在优化任何东西之前，先建立能客观判断"变好还是变坏"的评测集**。RAG 调参、Agent 改 prompt、换模型、上 rerank，每一步的依据都应该是同一套评测上的分数。

## 为什么"感觉变好了"不可信

- **选择性记忆**：新 prompt 跑了三个查询都更好，第四十个失败的你没注意
- **任务漂移**：这次测试用的查询和上次不同，对比无效
- **单点归因**：整体变好可能掩盖某类问题变差（中文查询变好，代码查询变差）

评测集的作用是把"测试"从随意的抽查变成**固定、可重复的回归测试**。对 RAG/Agent，它相当于单元测试之于普通代码（见 [pytest](../python/10-pytest-basics.md)）。
...
组成比例建议：**常规问题 60% + 边界/困难问题 25% + 应拒答负例 15%**。负例（库里没有的问题）尤其重要，它测的是"模型敢不敢说不知道"（见[生成与引用](../rag/05-generation-and-citations.md)）。
...
3. **分数要入库**：每次评测结果（日期 + 改动 + 三层分数）记下来，便于看效果趋势
