...
date: 2026-09-05
tags: [工具调用/MCP]
summary: function calling 不是模型"执行"函数——它只是输出一个结构化的调用请求，执行永远在你的代码里。理解这个分工，一切细节都对上了。
---

[最小 Agent 循环](03-minimal-agent-loop.md)已经在用 function calling 了，这篇把它作为独立协议讲透：schema 的写法、执行的信任边界、并行调用与流式。
...
关键认知：**模型从不执行任何东西**。它输出的是"我想调 get_weather，参数是杭州"这个**结构化意图**，执行、鉴权、错误处理全在你的代码里。这层分工决定了安全性设计：真正的权限控制在你的工具实现里，而不是祈祷模型"懂事"。
...
- **enum > 自由文本**：能用枚举圈死的参数绝不开放字符串
- **required 最小化**：必填越多，模型越容易瞎填；可选参数靠 description 引导
...
模型看到错误会自行修正参数重试——这是 Agent 自愈的机制基础。但**执行侧要设硬边界**：单工具超时、单轮最大调用数，防模型在同一个坑里无限重试。
