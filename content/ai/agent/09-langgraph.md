...
**2. Human-in-the-loop**：`interrupt()` 在任意节点暂停图执行，等待人审批后 `resume`，是工具调用确认门（见 [function calling 安全](06-function-calling.md)）的框架化实现，状态由 checkpointer 托管，不用自己写暂停恢复逻辑。
...
**不用/后用**：单循环 + 两三个工具的最小 Agent，手写 60 行更透明；纯 workflow 的固定流水线直接写代码，图是负担。判断依据回到 [workflow vs agent](01-workflow-vs-agent.md)。
