...
date: 2026-09-05
tags: [Agent设计模式]
summary: 一个 while 循环 + 工具注册表就是 Agent 的全部骨架。不用框架手写一遍，才知道 LangGraph 这类框架到底替你做了什么。
---

Agent 的核心机制很简单：**模型在循环里反复调用工具，直到不再调用为止**。60 行代码就能把这个骨架写完整。先手写再学框架，否则无法分辨框架的哪些复杂度是必要的。
...
就这么多。四个组成部分：**工具注册表**（名字→实现+schema）、**schema 声明**（模型靠它知道有什么工具、怎么传参）、**决策循环**（tool_calls 为空即结束）、**结果回填**（role=tool 消息）。
...
**终止条件必须有界。** `max_turns` 防死循环（模型反复检索同一个 query）；工具侧也应有自己的超时。终止判断靠模型"不再调用工具"；模型不停止，就说明 prompt 或工具有问题。

**状态就是 messages 列表。** 整个 Agent 的"记忆"就是不断追加的消息历史。理解这一点，[记忆与状态](04-memory-and-state.md)讲的所有问题（历史太长怎么办）都只是对这条列表的管理策略。

## 再看框架

LangGraph 解决的是这个骨架变复杂之后的痛点：状态持久化（循环崩了能恢复）、条件边与人工介入（循环中途暂停等人审批）、可视化与并发分支。如果这些痛点你还没有，60 行就是最好的架构；有了，再看 [LangGraph：状态机模型](09-langgraph.md)。
