...
date: 2026-09-05
tags: [Agent设计模式]
summary: prompt chaining、routing、并行化、orchestrator-workers、evaluator-optimizer——五个模式覆盖绝大多数 LLM 应用的结构设计，每个的成本与适用场景都不同。
---

这五个模式出自 Anthropic《Building Effective Agents》，是把"用模型搭系统"经验收敛成的可复用结构。它们全是 **workflow**（控制流由你写死），也正是这个属性让它们可调试、可预测。看懂这五个，市面上大多数"XX Agent 框架"的架构图都能对号入座。
...
- **注意**：中间步骤加"程序化闸门"（校验，不过就重试或终止）——链条越长越要防错误传播
- **代价**：延迟线性叠加
...
- **适用**：子任务独立、可以同时跑
- **变体**：sectioning（拆分处理）与 voting（同一问题跑多次取多数/共识——提升单点可靠性，代价是 N 倍成本）
...
- **边界**：拆解仍由模型决策，开始带 Agent 味道——控制不住时退回 chaining
...
- **不适用**：标准模糊的品味问题——评审模型自己也说不清"哪里不好"，循环空转

## 怎么用这五个模式

它们是**积木，不是互斥选项**：真实系统常是组合态（routing 分流后，kb_search 分支内部是 chaining，其并行步骤用 parallelization）。设计顺序：先画数据流草图 → 每个节点问"这步能预先画出来吗" → 能就是 workflow 模式，不能才考虑真 Agent（见 [workflow vs agent](01-workflow-vs-agent.md)）。
