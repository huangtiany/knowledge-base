...
date: 2026-09-05
tags: [中间件]
summary: 前端对 Nginx 不陌生：它托管 dist、转发 /api；换到后端视角，它还是负载均衡器。静态资源、反向代理、负载均衡三重身份的配置与常见问题。
---

前端已经在两处用过 Nginx：本地 devServer 的 proxy、部署时托管 dist 的静态服务器，都是它。放到后端视角，它还有第三重身份：**负载均衡器**。
...
这就是前后端分离部署的标准做法：**同域名下，/api 归后端，其余归前端**，没有跨域问题（浏览器只看到同源请求）。对照本地 devServer 的 `proxy: { '/api': ... }`，机制相同，只是发生在生产环境的独立进程里。所谓**反向代理**：代理的是"服务端"（用户不知道也不需要知道真实后端在哪），对照"正向代理"（代理的是客户端，如 VPN）。
...
默认轮询；`weight` 加权；`ip_hash` 按来源 IP 固定分配（Session 存本机内存时的会话保持方案；更现代的做法是 Session 放 [Redis](01-redis.md) 共享，就不需要粘性了）。Nginx 在最前面承接流量，单实例故障也能被摘除（`max_fails` 探活）。
...
**1. `proxy_pass` 尾斜杠决定路径怎么拼。** 这是 Nginx 反向代理最常见的问题：
...
**2. 后端拿到的"客户端 IP"是 Nginx。** 不加转发头，Spring Boot 里 `request.getRemoteAddr()` 永远是 Nginx 的内网 IP，限流、风控、日志全部失真。标准三件套：
...
注意 `X-Forwarded-For` 客户端可伪造第一段，服务端要取"从右往左第一个非可信代理"的 IP，不能直接取第一个。

**3. 默认超时和体积限制。** 后端跑长任务（导出报表、调用 LLM）超过 60 秒 → 浏览器收到 504，是 Nginx 的 `proxy_read_timeout` 默认值导致的，不是后端挂了；上传大文件报 413，是 `client_max_body_size` 默认 1MB 导致的：
...
**4. location 匹配不是从上到下。** 优先级：`=` 精确匹配 > `^~` 前缀（跳过正则）> `~` 正则（按出现顺序）> 普通前缀（取最长）。把正则和前缀混写时，"先写的先生效"是常见错觉，拿 `nginx -T` 看最终合并结果。
...
这张图就是前后端分离项目的部署全景：前端工程师要理解代码如何被服务，Nginx 是绕不开的一环。
