...
date: 2026-09-05
tags: [工程实践]
summary: pom.xml 就是 package.json，依赖坐标、生命周期、仓库的对应关系几乎一比一；最大的差异是中央仓库慢，国内要配镜像。
---
...
最大的结构差异：**依赖不放在项目目录里**，而是统一存本地仓库，项目间共享，避免了 node_modules 式的目录膨胀；代价是依赖的版本冲突要 Maven 自己仲裁（同一 artifact 多版本时选"最近的"传递路径，冲突显式用 `<exclusions>` 排除）。
...
- `mvn install`：打包并把**自己装进本地仓库**（供本地其他项目依赖，相当于把"发布到私有 npm"变成"拷进共享文件夹"，用于内部项目）

执行后面的阶段会自动执行前面所有阶段，是一条固定流水线，不是任意脚本。要跑自定义任务用插件（如 `spring-boot:run`，对照 npm scripts 里挂依赖包的命令）。

## 仓库与镜像：国内必配

依赖的解析顺序：**本地仓库 → 私服（有则用）→ 中央仓库**。中央仓库在国外，不配镜像时第一次拉依赖会非常慢。用阿里云镜像（Maven 的 `settings.xml`，IDEA 和命令行共用）：
