...
date: 2026-09-05
tags: [工程实践]
summary: 规范不是为了好看，是团队协作的通信协议。公司规范以内部为准，但公开的行业基准——《阿里巴巴 Java 开发手册》——覆盖了 80% 的日常判断，第一天就照它写。
---

入职后公司有编码规范（没有的话更要立），但**规范意识可以先于文档养成**。这篇以公开的行业事实标准——《阿里巴巴 Java 开发手册》——为基准，收转型期最容易违反的条款。原文免费（GitHub 开源，含 IDEA 扫描插件），值得通读一遍。
...
3. **POJO 类（数据对象）布尔字段不要加 is 前缀**：`isDeleted` 在序列化框架里会出灾难（部分框架把它读成 deleted），用 `deleted`
4. **杜绝无意义缩写**：`getInfo`、`data2`、`temp`——半年后自己都看不懂的名字，就是坑队友
...
- **多线程共享的可变集合用 ConcurrentHashMap**，不要 HashMap 裸奔
...
2. **入乡随俗优先**：接手老项目时，跟项目的既有风格走（哪怕与手册冲突）——"项目组特有规范以负责人为准"是行业现实；重构风格是独立的技术决策，不搭规范的车
3. **CR 是学习不是考试**：被 CR 指出规范问题是最便宜的纠错时刻，比线上事故便宜一百倍
