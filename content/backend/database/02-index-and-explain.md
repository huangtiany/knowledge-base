...
date: 2026-09-05
tags: [数据库]
summary: 索引是后端性能优化第一课，B+ 树让百万行查询走毫秒级；explain 用来确认查询走没走索引，索引失效场景要当 checklist 记。
---
...
索引 = 排好序的查找结构（MySQL InnoDB 用 **B+ 树**）。直觉类比：字典的目录，按拼音排序后二分查找定位，不用一页页翻。
...
理解 explain 输出前还需要一个概念：**回表**。InnoDB 里数据行本身按主键组织成一棵 B+ 树（聚簇索引），`idx_user_city` 这种二级索引的叶子节点存的是"索引列值 + 主键"，不是整行。所以 `SELECT * FROM users WHERE city='杭州'` 要先在二级索引树里找到主键，**再回聚簇索引树查整行**，这一步就是回表，一次查询可能回表几十次。覆盖索引之所以快，就是"列都在索引里，不用回表"。
...
- **慢查询落地流程**：慢日志抓到 SQL → explain 定位 → 改写或加索引 → 用 explain 复验
