...
date: 2026-09-05
tags: [Java基础]
summary: 写惯 JS 的人学 Java，最大的跃迁不是语法而是"编译期"——类型必须声明、错误必须面对。这篇建立强类型 + 面向对象的心智模型。
---

写惯 JS 的人上手 Java，语法表面相似（大括号、分号、if/for 几乎一样），真正的跃迁在两处：**编译期**（类型不声明不让过、受检异常必须处理）和**面向对象是默认组织方式**。这篇把心智模型立起来。
...
与 JS 的本质区别：类型是**编译期契约**。`age = "abc"` 在 JS 里运行时才炸，Java 直接编译失败——编辑器当场红叉。习惯这个"被编译器管着"的感觉，是转型第一课。

八个基本类型（int、long、double、boolean、char…）直接存值；其余一切都是对象（引用类型），存的是引用。对照 JS：number 一把梭，Java 把整数细分成 byte/short/int/long——日常业务记 `int`（默认）和 `long`（ID 场景，雪花 ID 超出 int 范围）即可。
...
**JS 的 `===` 直觉在 Java 的字符串上会翻车**：`==` 比较对象身份，`equals` 比较内容。这是所有 Java 新手的第一个坑，规则一句话：比内容，永远 `equals`。
...
方法重载（同名、参数不同）是 Java 的常规操作，JS 里只能靠默认参数模拟。程序入口是固定的 `public static void main(String[] args)`——对照 JS 的顶层代码或 `main()`，Java 一切从这开始。
...
PayStrategy p = new AlipayPay();        // 面向接口编程 —— 策略模式的地基
p.pay(100);
```

`interface` 比 TS 的 interface 更"实"：它有运行时存在，是实现契约的强制手段。**面向接口编程**是 Spring 生态无处不在的味道，从现在开始养成。
