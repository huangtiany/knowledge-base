...
date: 2026-09-05
tags: [工程实践]
summary: IDEA 对照 VSCode 是"重厂重车"；Linux 命令是后端的生存技能——查日志、看进程、读权限三件事每天都要用。两块都按"高频优先"收。
---

两件工具层面的生存装备：IDEA（对照 VSCode，但为 Java 深度优化）和 Linux 常用命令（后端程序跑在 Linux 服务器上，命令行是排障现场）。都按高频优先收。
...
- `Alt + Insert`：生成代码（getter/setter/构造器/重写方法）——对照 VSCode 的快速修复升级版
- `Shift + Shift`：全局搜一切（类、文件、符号）——对照 Ctrl+P 的强化版
- `Ctrl + Alt + ←/→`：跳转历史（读代码神器，沿调用链跳进跳出后回退）
- `Ctrl + B`：跳到定义；`Ctrl + Alt + H`：查看调用层级——**读陌生代码的主要方式**是"跳转 + 调用层级"，比全局搜索高效
- `Shift + F6` 重命名重构：改名后**所有引用同步更新**（Java 静态类型的红利，JS 重构不敢这么闭眼）
...
生产日志动辄几 GB，**永远不要 cat 整个文件**——tail + grep 组合拳是 80% 的排障动作。

### 权限：drwxr-xr-x 怎么读

`ls -l` 输出开头的十位：第 1 位是类型（`d` 目录、`-` 文件），后九位三三一组——**所有者 / 同组 / 其他人**，每组是 `rwx`（读/写/执行）：
...
对照前端的任务管理器杀 node 进程，但现场是 SSH 终端 + 没有图形界面。有了这几条 + 日志三件套，远程服务器上你就能活下来；更深的（systemd、防火墙、网络排查）遇到再查。
