...
date: 2026-09-05
tags: [数据库]
summary: 转账不能只扣不加——事务保证"要么全成，要么全败"。隔离级别决定并发下你能忍受哪种脏数据，死锁则是加锁顺序的锅。
---

经典开场仍是转账：A 扣 100、B 加 100，两个 UPDATE 必须同生共死——A 扣了钱、B 没到账，钱就凭空消失了。**事务**（Transaction）保证一组操作要么全部成功、要么全部回滚，像什么都没发生过。
...
对照前端：`Promise.all` 的"要么全部 resolve 要么 reject"只像了原子性；数据库的四特性由引擎（InnoDB）保证，你通过 SQL 使用它。
...
记忆锚点：**MySQL 默认 RR（可重复读）**，业务代码通常不用动它；遇到"同一事务内两次查询结果不一致"的 bug，先查是不是用了 READ COMMITTED 或事务边界划错了。

## MVCC 一句话版

InnoDB 实现 RR 靠 **MVCC**（多版本并发控制）：每行数据藏了版本号，事务按开始时刻的快照读旧版本——读写互不阻塞。这是"读不加锁还能隔离"的秘密，细节用到再深挖（小林 coding 的图解最清楚）。
...
1. **按固定顺序访问资源**（如统一按 id 升序更新）——打破"互相等"
