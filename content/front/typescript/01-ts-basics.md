...
date: 2026-09-06
tags: [TypeScript]
summary: TS 的价值在于让重构有信心、提示有精度。基础标注、函数类型、可辨识联合与收窄、any/unknown/never 的分工。
---

TS 不是「给 JS 加注释」，它的核心价值有两点：**重构有信心**（改名/改签名时编译器把所有受影响处标红）和**提示有精度**（IDE 补全的每个字段都有依据）。下文覆盖基础标注、函数类型、类型收窄与 any/unknown/never 的分工。
...
`type` 与 `interface` 九成场景可互换：**联合、元组、映射、工具类型用 type；对象形状 + 声明合并（如给 window 补字段）用 interface**。团队里统一一种直觉比纠结差异更重要。
...
// 可辨识联合：用共同字面量字段做标签，switch 自动收窄，建模状态机的标准姿势
type State =
  | { status: "loading" }
...
可辨识联合是日常收益最高的做法：接口状态（loading/ok/error）建模成联合后，**漏处理某个分支编译器直接报错**，比 if-else 加注释可靠一个量级。

## any / unknown / never：逃生门的分级

```ts title="escapes.ts"
let a: any = JSON.parse(text);    // 关闭所有检查，并传染给每一处使用
let u: unknown = JSON.parse(text); // 受限的 any：用之前必须收窄/断言
if (typeof u === "object" && u !== null) { /* 这里面才能当对象用 */ }

function fail(): never { throw new Error("boom"); }  // 永不正常返回：兜底分支的类型
```

- **any 关闭所有检查**：类型检查对它完全失效，且顺着赋值传播。代码库里 any 越少，重构越敢动手
- **unknown 是受限的 any**：接住不确定的外部数据（JSON.parse、第三方回调），使用前必须先验证；接外部数据默认用 unknown
- **never 用于「不可能」的分支**：穷尽性检查用，给 switch 加 `default: const _exhaustive: never = state;`，将来 State 加新分支时这里立刻编译报错

## tsconfig 起步：新项目默认 strict
...
`strict: true` 引入的最大约束是**严格空值检查**：`string | null` 不收窄不许用，老代码里一半的 `Uncaught TypeError: cannot read property of undefined` 在编译期就被拦下。历史项目的迁移策略：先开 strict 再逐文件修，不要为了省事大面积用 `any`。工具链层面（lint 与 TS 的配合）见[质量工具链篇](../engineering/02-quality-toolchain.md)。

## 小结

**导出必标、局部靠推**；**联合类型靠收窄用**，状态建模用可辨识联合；**unknown 代替 any**，never 做穷尽性检查。类型的复用与变换见[泛型与工具类型](02-generics-and-utility-types.md)。
