...
[入门篇](04-tauri-core.md)跑通了 invoke 与 command，本篇处理从能跑到能发布的全部工程问题：**权限模型**（v2 最重要的变化）、**插件体系**（优先复用现成插件）、**事件与状态**（真实应用的通信形态）、**打包签名与自动更新**（分发前的必要准备）。

## 安全模型：capabilities 与 permissions

Tauri v2 把前端能调什么变成显式授权，这是它与 Electron 安全模型最大的分野：
...
- **默认全部拒绝，按需最小授权**：官方插件的能力都被拆成细粒度 permission，前端能调 `fs.read` 不代表能删文件，能访问 `$APPDATA` 不代表能扫全盘
- 对 Web 的类比：这就是桌面版的 CORS，跨过 IPC 边界的每个调用都要经过授权检查，[入门篇](04-tauri-core.md)里 `invoke` 失败报 "not allowed" 时先查 capabilities
- 自定义 command 同样受这套体系管：插件化的能力用 permission 声明，应用内命令按窗口授权

## 插件矩阵：官方插件优先

系统能力不要手写，先查官方/社区插件有没有现成实现：
...
invoke 是前端主动问，真实应用还需要 Rust 主动推送，以及跨命令共享状态：
...
- **代码签名是分发的前置项**：Windows（EV 证书，否则 SmartScreen 的警告会让多数用户放弃安装）、macOS（Developer ID + 公证 notarization，否则 Gatekeeper 拦截）；签名配置进 CI，本地不碰私钥
- **自动更新**的完整流程：updater 插件轮询 endpoints → 比对版本 → 下载增量产物 → 校验签名 → 重启应用。服务端只需要一个带版本元数据的静态 JSON + 更新包，用 GitHub Releases 或对象存储就能起步
- 版本策略提醒：桌面应用没有 Web 的「刷新即最新」，用户停留在旧版本数月是常态，**前端接口兼容与灰度发布的纪律**要按客户端软件的尺度执行（这点与[部署监控篇](../engineering/05-deploy-and-monitoring.md)的 Web 心智相反）

## 小结

Tauri 工程化的四个要点：**capabilities 最小授权**（v2 安全模型的基础）、**插件优先复用官方矩阵**、**通信三形态按场景选**（invoke/emit/channel）、**签名与更新是分发门槛**（准备要早于功能开发启动）。
