...
date: 2026-09-06
tags: [浏览器原理]
summary: 渲染流水线 DOM → CSSOM → 布局 → 绘制 → 合成——懂这条链，才知道重排重绘为什么贵、transform 为什么快、script 为什么会堵页面。
---

前端代码跑在浏览器里，性能上限由渲染机制决定。这篇按「一个 URL 如何变成屏幕上的像素」为主线走一遍渲染流水线，落到三个工程结论：重排为什么贵、`transform` 为什么快、`script` 标签为什么放错位置会白屏。加载阶段的网络侧见[HTTP 缓存与浏览器存储](02-http-cache-and-storage.md)，指标度量见[性能优化](../performance/01-performance-metrics.md)。
...
- **DOM 与 CSSOM 并行构建**，二者合出渲染树——所以 CSS 阻塞渲染（CSSOM 没好就不能首绘），这就是「关键 CSS 内联、其余异步」的原理
...
浏览器把页面切成多个**层**（layer），合成线程把各层栅格化后拼接上屏——这步不占主线程。`transform/opacity` 动画能被提升到独立层，走「合成器动画」，即使主线程繁忙（长任务执行中）也能流畅跑。

```css title="layers.css"
.modal { will-change: transform; }  /* 提示浏览器提前建层：用于已知要动画的元素 */
```

`will-change` 是**提示不是开关**：给几百个元素都加会导致内存暴涨，反而变慢。原则是「给确定要动画的少量元素用，动画结束移除」。判断一个元素是否真的建了层，Chrome DevTools 的 Layers 面板可以直接看。

## 长任务与交互响应

渲染是主线程的事，**超过 50ms 的任务就是长任务（Long Task）**——期间点击、输入全部排队，表现为「卡」。
...
- 交互性能指标 INP 直接度量「输入到下一帧」的延迟，治理手段就是压缩长任务——见[性能优化篇](../performance/01-performance-metrics.md)

## 小结

流水线思维收拢成三句话：**改几何最贵、改颜色中等、transform/opacity 最便宜**；**script 要 defer，CSS 要关键内联**；**长任务是一切交互卡顿的元凶**。理解了渲染侧的开销来源，度量与优化清单见[性能优化](../performance/01-performance-metrics.md)；页面加载的第一公里（缓存）见[HTTP 缓存与浏览器存储](02-http-cache-and-storage.md)。
