...
当项目长出「组件库 + 工具库 + 多个应用」时，多仓库的痛点开始放大：公共包改一行要发版再升级、版本漂移、规范各自维护。Monorepo 的答案是**一个仓库装下所有包，工具链保证它们像分开的一样协作**。这篇讲清两层基建：pnpm workspace 管依赖，Turborepo 管任务。包管理器基础见[质量工具链](02-quality-toolchain.md)。
...
- **`workspace:` 协议是 monorepo 的灵魂**：包之间互相引用走本地链接，没有发版环节——这是与 multirepo 最本质的差别
- `pnpm -r build` 递归执行所有包的 build，并**按依赖拓扑排序**（utils 先于 ui 先于 web）；`--filter @acme/web...` 只跑某包及其依赖
- 依赖版本统一用 **catalog**（pnpm 9.5+）：`pnpm-workspace.yaml` 里声明一份版本表，各包写 `catalog:` 引用——治理「同一个 React 十个版本」的经典问题

## Turborepo：任务编排与缓存

包一多，`pnpm -r build` 的全量执行就成了 CI 的瓶颈——Turbo 补上「**只构建需要构建的**」：
...
- 微前端的边界认知：monorepo 解决「代码放哪、怎么共享」，微前端（qiankun/Module Federation）解决「多个独立团队的应用拼进一个页面运行时」——组织问题先于技术问题，多数场景 monorepo + 路由拆分就够了

## 小结

Monorepo 的心智两句话：**workspace 协议消灭「包之间的发版环节」**（改动即时可见），**任务缓存消灭「没改也要构建的浪费」**（Turbo 指纹命中回放）。它不是默认选项——多包同速演进之日再上，上来就把 workspace + Turbo + catalog 三件套配齐。
