...
date: 2026-09-06
tags: [React生态]
summary: App Router 时代的 Next.js：RSC 把「不进 bundle 的组件」变成默认，文件约定长出路由与布局，缓存模型与 Server Actions 补全全栈闭环——React 官方推荐的全栈答案。
---

Next.js 是 React 官方文档推荐的元框架，App Router（13+ 版本）之后它不再是「React + 路由 + 打包」的脚手架，而是一次架构转向：**React Server Components（RSC）成为默认**——组件默认在服务端渲染、不进客户端 bundle，需要交互的部分显式标注 `'use client'`。Vue 侧的同构答案见[Nuxt](../vue/04-nuxt.md)。这篇按 App Router 的心智主线走。
...
- 判断法则一句话：**有交互（state/effect/事件）才 'use client'，静态展示一律留在服务端**——把交互压在叶子节点，bundle 就最小

## 缓存模型：Next.js 最需要小心的部分

App Router 的性能来自激进的缓存，心智模型是四层：
...
经验法则：**默认吃缓存（快），在 mutation 点用 `revalidateTag/Path` 主动失效**；实时性强的页面用动态 API 或 `no-store` 显式跳过。缓存是 App Router 的高频翻车点——「为什么改了没生效」九成在这张表里。
...
- 什么时候用：内容站/电商/任何需要 SEO 与首屏的场景；纯后台管理系统 SPA 足够——Next 也能跑（全客户端组件），但那是「用它的路由而不是 RSC」
- 与 Nuxt 对照：理念高度同构（文件路由/混合渲染/服务端目录），差异在 RSC 这层——React 把渲染拆进组件树，Vue 的组合式模型仍在客户端组件心智内

## 小结

App Router 的主线三句话：**组件默认在服务端**（`'use client'` 是少数派边界），**缓存默认全开**（靠 revalidate 主动失效），**数据读写都有内建通路**（RSC 直取 + Server Actions 直写）。React 全栈拼图的最后一块就此补齐——部署细节见[部署与前端监控](../engineering/05-deploy-and-monitoring.md)。
