...
date: 2026-09-06
tags: [JavaScript]
summary: 单线程为什么能不卡？答案在事件循环。从宏微任务到 Promise 链式规则，再到 async/await 的并发陷阱——前端的心跳机制一篇讲透。
---

JS 是单线程的，却要处理网络请求、定时器、用户输入——「单线程为什么不卡」的答案就是事件循环。这篇从调度模型讲到 Promise 的链式规则，最后落到 async/await 最容易踩的并发陷阱。多线程心智的对照版可以看后端那篇[多线程与并发](../../backend/java-basics/04-concurrency.md)：Java 用线程池并行，JS 用事件循环并发，思路完全不同。

## 事件循环：调用栈、宏任务与微任务

事件循环的骨架：**执行完同步代码（调用栈清空）→ 清空全部微任务 → 取一个宏任务 → 再清微任务 → 循环**。
...
- **宏任务**：setTimeout/setInterval、I/O、UI 渲染、事件回调——一个一个来，之间可能插入渲染
...
Promise 是一个三态状态机：**pending → fulfilled / rejected，一旦落定不可再变**。链式调用的关键规则是「**每个 then 返回一个新 Promise**」，值一路向后传，错误一路向后冒：
...
常见用法：互不依赖的请求用 `all` 并行；「全部要、单个失败不炸」用 `allSettled`；「请求 + 超时竞争」用 `race`。
...
- **forEach 不认 await**：`arr.forEach(async () => await …)` 只是发出一堆不等待的任务；要么 for...of，要么 Promise.all

## 实战三式：防抖节流、取消与竞态

```js title="patterns.js"
// 竞态保护：慢的旧请求覆盖新的新请求——搜索框标配
let seq = 0;
...
- **竞态**：同一接口的并发请求，结果乱序到达——序号比对或 AbortController 二选一，搜索/联想输入必备
- **防抖节流**：防抖「停止触发才执行」（搜索联想），节流「固定频率执行」（滚动/resize），实现见[闭包篇](01-scope-closure-prototype.md)
- 把这三式组合起来，就是本站搜索框的完整实现：防抖 500ms + 竞态保护

## 小结

事件循环回答「单线程为什么不卡」，Promise 的状态机与链式规则回答「异步代码怎么组织」，async/await 是可读性糖——但**并行与串行的判断永远要自己做**。异步慢的下一站是网络与缓存，见[HTTP 缓存与浏览器存储](../browser/02-http-cache-and-storage.md)；卡顿的度量与治理则交给[性能优化](../performance/01-performance-metrics.md)。
