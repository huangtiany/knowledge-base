...
date: 2026-09-06
tags: [JavaScript]
summary: ES6 是 JS 的分水岭——let/const、解构、箭头函数、模块从此定形。这篇把 2015 之后值得天天用的语法与 ESM 模块系统收成一页速查。
---

2015 年的 ES6 是 JS 的分水岭：此前是「语言缺陷靠模式补」，此后是「语法表达直接到位」。日常写码九成语法落在这篇速查里，偶尔用到的 ES2020+ 新东西也一并收录。目标不是背特性清单，而是把「这段代码在表达什么」的阅读速度提上来。
...
`?.` 与 `??` 是 2020 年标准（ES2020），如今可放心使用。特别注意 `??` 与 `||` 的差异是真实事故源：音量 0、开关 false 这类合法值会被 `||` 吃掉。
...
ES6+ 的主线是**表达能力升级**：解构、箭头函数、模板串消灭样板代码，Map/Set 补齐数据结构，`?.`/`??` 治理空值。模块层记住一个词——**静态**：它是 tree-shaking、按需加载、dev 直跑这些工程能力的根。语言的单线程模型下异步怎么运转，见下一篇[异步与事件循环](03-async-and-event-loop.md)。
