...
| 站内搜索 | 只搜文章标题：构建期生成标题 JSON，前端子串匹配，不做全文索引 |
...
标准 Markdown 相对链接（`[Transformer 结构拆解](../llm-basics/transformer.md)`），不引入 `[[wikilink]]` 自定义语法；任何编辑器原生可写可跳。
...
| 文章搜索 | 构建期生成全站标题 JSON，前端子串匹配（中文无需分词），不做全文索引 |
